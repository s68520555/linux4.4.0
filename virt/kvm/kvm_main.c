...
/*test code 1*/
static void write_file_self(struct kvm_memslots *slots, int number, struct file *filpr)
{
	...
	if(filpr == NULL) {
		printk("open file!\n");
                char filename[5000] = "/home/tsai-te-yu/kvmaddress/address.txt";
		printk("%s\n", filename);
                filpr = filp_open(filename, O_CREAT|O_RDWR, 0644);
		if (IS_ERR(filp)) {
        		printk("open error\n");
        	}	
        	else {
		...
		if(slots->memslots[j].npages == 0)/*Just print these slots which gfn is not 0.*/
			continue;	
		printk("%d %llu %lu %lu\n", number, slots->memslots[j].base_gfn, slots->memslots[j].userspace_addr >> 12, slots->memslots[j].npages);
...
static void kvm_destroy_vm(struct kvm *kvm)
{
	...
	//kvm_mem_slot_print(kvm);

	/*test code 4*/
	struct kvm *kvm_list = kvm;
	...
	int number = 0;
		...
		printk("number: %d\n", number);
                for(i = 0; i < KVM_ADDRESS_SPACE_NUM; i++) {
                write_file_self(kvm_list->memslots[i], number, filp);
                }
		number++;
        }
